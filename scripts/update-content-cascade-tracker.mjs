...
例えば、ダイナミックプライシング。
毎日触ってるか、放置してるかで、稼働率は月単位で 5-10 ポイント変わる。

人力で 30 物件全部を毎日触るのは無理。

AI に毎日触らせて、エスカレーションは社内で見る。
この前提があって初めて、8% で Airbnb スーパーホスト多数認定の運営力が出る。

---

要は、業界相場 20% は「人力 × 5-10 物件 / 人」のコスト構造。

AI に回させて 30-50 物件 / 人にできれば、8% で黒字になる。
...
これで 1 人が見られる物件数が 5-10 → 30-50 まで上がる。
AI を真ん中に置いた事業構造で、8% でスーパーホスト品質を維持できる。
...
const PIPELINE_ROWS = [
  ...
  ["T-LAUNCH", "@tenichiliu", "業界トレンド + オーナー成功事例",
    ...
    "✨ v4: AI 中心の事業構造を明示 ｜ 🔴 PR TIMES URL を [PR TIMES Link] に挿入要",
    DRAFT_LAUNCH_T],
...
async function main() {
  ...
  console.log(`✅ Pipeline v4 updated: ${PIPELINE_ROWS.length} rows (AI mention + Japanese SNS style)`);
