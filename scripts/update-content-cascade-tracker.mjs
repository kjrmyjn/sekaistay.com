...
最初は Airbnb のみだったところに、Booking.com・一休・Trip.com を追加。

ハイエンド帯は、特に Trip.com（中華圏のインバウンド）と一休（国内富裕層）からの流入が強い。
媒体ごとに刺さる顧客層が違うので、面を増やすだけで稼働率が上がる典型例です。
...
価格は Pricelabs + AI で毎日チューニング。
ハイエンド帯は需要の波がシビアで、日次で動かさないと取りこぼします。
