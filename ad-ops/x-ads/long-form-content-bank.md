...
| 火 5/19 | @jirosan | #3（DP 毎日触る） | 12:00 | 現場ノウハウで「うちは仕組み化が違う」を可視化 |
| 水 5/20 | @ss_unei_chan | #5（家具失敗談） | 19:00 | 中の人感で親近感獲得・フォロー誘導 |
| 木 5/21 | @tenichiliu | #2（受賞振り返り） | 08:00 | 受賞実績でローンチ前夜の信頼度ブースト |
| 木 5/21 | @tenichiliu / @jirosan | ローンチ予告通常ツイート | 20:00 | 「明日大事な発表があります」型のティーザー |
| **🚀 金 5/22** | **@tenichiliu + @jirosan** | **長文B #LAUNCH-T / #LAUNCH-J（要作成）** | **10:00 同時** | **プロダクトローンチ + PR TIMES 当日砲火** |
| 金 5/22 | @ss_unei_chan | 通常ツイート（中の人視点歓喜・短文） | 11:00 | ローンチの裏側を運営チーム視点で |
| 土 5/23 | @jirosan | #4（民泊新法 2026） | 12:00 | ローンチ余韻・ノウハウ系で滞在時間獲得 |
| 日 5/24 | @ss_unei_chan | #6（オーナーダッシュボード） | 19:00 | ローンチしたプロダクトと相性◎の事例で締め |

### 5/22 ローンチ長文B（#LAUNCH-T / #LAUNCH-J）— 作成待ち

ローンチする具体プロダクト・差別化軸の確定後にドラフト追記。テンイチ版（経営者・ビジョン軸）とジロー版（現場・実証軸）で同日同時刻に同時砲火する設計。

テンプレ骨子:

```
（フック）
今日、SEKAI STAY は [プロダクト名] をローンチしました。
（背景・なぜ作ったか - 3行）
（具体的に何が変わるか・既存代行との違い - 8-12行）
（実証データ or 数字 - 3-5行）
（学び/思想 - 箇条書き 3-5個）
（締め - オーナーへの呼びかけ）

詳細は PR TIMES → [PR Link]
→ https://sekaistay.com/switch?utm_source=x&utm_medium=organic&utm_campaign=launch&utm_content=launch_tenichi
```
